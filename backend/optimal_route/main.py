...
if __name__ == '__main__':
    ...
    og, ids = nxt.create_graph(berlinCenter[0], berlinCenter[1], cafes)
    G = og.graph
    path = nxt.dijkstra_path(G, ids[0], ids[1])
    #len_path = nxt.dijkstra_path_length(G, ids[0], ids[1])
    print(f'Path from {ids[0]} to {ids[1]}:')
    print(path)
    pt.plot_path(cafes, og, path)
